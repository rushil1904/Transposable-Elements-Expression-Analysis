...
file_uuids = colnames(RE_exon_1_raw_counts)
head(file_uuids)

#write.csv(file_uuids, "RE_exons.csv", row.names = FALSE)

new_column_names <- (barcodes_RE_exons$`0`)
colnames(RE_exon_1_raw_counts) <- new_column_names
View(RE_exon_1_raw_counts)

genomic_idx <- match(rownames(clinical), colnames(RE_exon_1_raw_counts))
genomic_idx

dataset_ordered <- RE_exon_1_raw_counts[, genomic_idx]
all(rownames(clinical) == colnames(dataset_ordered))

#saveRDS(dataset_ordered, file="Dataset_RE_exons_TEEAnalysis.rds")
...
write.csv(dds_res_sig, file="significant_exons.csv")
write.csv(dds_res, file="all_significant_exons.csv")
