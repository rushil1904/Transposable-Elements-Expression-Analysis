...
column_names <- colnames(Paired_counts_introns_final)
paired_clinical <- clinical[column_names,]
paired_intron_counts <- RE_intron_1_raw_counts[,column_names]
genomic_idx <- match(rownames(paired_clinical), colnames(paired_intron_counts))
genomic_idx

dataset_ordered <- paired_intron_counts[, genomic_idx]
...
write.csv(dds_res_sig, file="significant_introns_paired.csv")
write.csv(dds_res, file="all_significant_introns_paired.csv")
