...
na_rows <- which(is.na(clinical$paper_mRNA.cluster))
na_rows
mRNA_cluster_clinical_df <- clinical[-(na_rows),]
View(mRNA_cluster_clinical_df)
mRNA_cluster_dataset_ordered <- subset(dataset_ordered, select=-(na_rows))
View(mRNA_cluster_dataset_ordered)
all(rownames(mRNA_cluster_clinical_df) == colnames(mRNA_cluster_dataset_ordered))

mRNA_cluster_clinical_df$paper_mRNA.cluster <- as.factor(mRNA_cluster_clinical_df$paper_mRNA.cluster)

#Setting the levels
mRNA_cluster_clinical_df$paper_mRNA.cluster <- relevel(mRNA_cluster_clinical_df$paper_mRNA.cluster, ref = "Basal_squamous")

#write.csv(clinical, file="clinical.csv")
#DESeq2
dds <- DESeqDataSetFromMatrix(countData = round(mRNA_cluster_dataset_ordered),
                              colData = mRNA_cluster_clinical_df,
...
View(vsd_cor)
class(vsd_cor)
pheatmap(vsd_cor, annotation = select(mRNA_cluster_clinical_df, paper_mRNA.cluster))

#Principal Component Analysis
plotPCA(vsd, intgroup="paper_mRNA.cluster")
...
dds_res <- results(dds,
                   contrast = c("paper_mRNA.cluster", "Basal_squamous", "Luminal_papillary"),
                   alpha = 0.05,
...
annotation_c <- dplyr::select(mRNA_cluster_clinical_df, paper_mRNA.cluster)
...
write.csv(dds_res_sig, file="significant_intergenic_mRNA_cluster.csv")
write.csv(dds_res, file="all_significant_intergenic_mRNA_cluster.csv")
