...
which( colnames(clinical)=="paper_ABSOLUTE.call.status")

na_rows <- which(is.na(clinical$paper_ABSOLUTE.call.status))
na_rows
tumor_cell_purity_clinical_df <- clinical[-(na_rows),]
View(call_status_clinical_df)
tumor_cell_purity_dataset_ordered <- subset(dataset_ordered, select=-(na_rows))
View(tumor_cell_purity_dataset_ordered)
all(rownames(call_status_clinical_df) == colnames(tumor_cell_purity_dataset_ordered))
...
tumor_cell_purity_dataset_ordered <- subset(tumor_cell_purity_dataset_ordered, select=-(na_rows_call_status))
...
#DESeq2
dds <- DESeqDataSetFromMatrix(countData = round(tumor_cell_purity_dataset_ordered),
...
write.csv(dds_res_sig, file="significant_raw_call_status.csv")
write.csv(dds_res, file="all_significant_raw_call_status.csv")
