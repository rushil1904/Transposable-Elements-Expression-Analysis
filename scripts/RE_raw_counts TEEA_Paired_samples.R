...
column_names <- colnames(Paired_counts_raw_final)
paired_clinical <- clinical[column_names,]
paired_raw_counts <- RE_all_1_raw_counts[,column_names]
genomic_idx <- match(rownames(paired_clinical), colnames(paired_raw_counts))
genomic_idx

dataset_ordered <- paired_raw_counts[, genomic_idx]
...
write.csv(dds_res_sig, file="significant_paired.csv")
write.csv(dds_res, file="all_significant_raw_paired.csv")
