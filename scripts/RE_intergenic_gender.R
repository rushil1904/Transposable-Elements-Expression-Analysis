...
na_rows <- which(is.na(clinical$paper_gender))
na_rows
gender_clinical_df <- clinical[-(na_rows),]
View(gender_clinical_df)
gender_dataset_ordered <- subset(dataset_ordered, select=-(na_rows))
View(gender_dataset_ordered)
all(rownames(gender_clinical_df) == colnames(gender_dataset_ordered))

#saveRDS(gender_dataset_ordered, file="Dataset_RE_raw_counts_GENDER.rds")
#write.csv(dataset_ordered, file="dataset_raw.csv")

gender_clinical_df$paper_gender <- as.factor(gender_clinical_df$paper_gender)

#Setting the levels
gender_clinical_df$paper_gender <- relevel(gender_clinical_df$paper_gender, ref = "MALE")

#write.csv(clinical, file="clinical.csv")
#DESeq2
dds <- DESeqDataSetFromMatrix(countData = round(gender_dataset_ordered),
                              colData = gender_clinical_df,
                              design = ~paper_gender)
...
View(vsd_cor)
class(vsd_cor)
pheatmap(vsd_cor, annotation = select(gender_clinical_df, paper_gender))

#Principal Component Analysis
plotPCA(vsd, intgroup="paper_gender")
...
dds_res <- results(dds,
                   contrast = c("paper_gender", "MALE", "FEMALE"),
                   alpha = 0.05,
...
annotation_c <- dplyr::select(gender_clinical_df, paper_gender)
View(annotation_c)
...
write.csv(dds_res_sig, file="significant_intergenic_gender.csv")
write.csv(dds_res, file="all_significant_intergenic_gender.csv")